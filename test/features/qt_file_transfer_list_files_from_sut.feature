...
@qt_linux
  Scenario: List files from sut with list_files_from_sut method
    ...
    And exception is not thrown
    And I can delete the test files
    When I execute "@file_list=@sut.list_files_from_sut(:from => '/usr/share/temp/test_copy/')"
    And exception is not thrown
    Then the files are listed from "C:\temp\test_copy\"
    And I test code "@sut.delete_from_sut(:from => '/usr/share/temp/test_copy/', :file => '*.txt')"
    And exception is not thrown
