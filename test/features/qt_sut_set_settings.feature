...
@qt_linux @qt_meego
  Scenario: Test setting a setting using path to ini file
    Given I have default sut
    When I execute "@sut.set_settings({:fileName => '/etc/qt_testability/qt_testability.ini', :format => 'ini'},{ :test_setting => 'foobaar'})"
    Then the test_setting value foobaar can be read from /etc/qt_testability/qt_testability.ini
